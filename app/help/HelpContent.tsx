...
const content = {
  ja: {
    ...
    concepts: [
      ...
      { title: "論理ボタンと出力", paragraphs: ["論理ボタンはCOIN、START、方向、A〜Lの18個です。ゲーム基板出力は1P側と2P側にそれぞれCOIN、START、方向、A〜Fの12個があります。G〜Lは直接対応する基板出力を持たず、マクロやステートセレクタの操作に使える仮想ボタンです。", "1つの論理ボタンから複数の基板出力を同時に出せます。たとえば論理ボタンGを、1P側のA＋Bや2P側のAへ割り当てられます。複数の論理ボタンやマクロが同じ出力を生成した場合は、すべてOR合成されます。2P出力はプロファイル上部の2P設定を有効にした時だけ編集画面へ表示されます。"] },
