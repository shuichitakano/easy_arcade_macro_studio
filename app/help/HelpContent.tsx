...
const content = {
  ja: {
    ...
    concepts: [
      ...
      { title: "ループ同期", paragraphs: ["通常の反復再生では、ボタンを押したフレームがマクロのtick 0になります。ループ同期ではボタンを押した時刻から数えず、EASY ARCADE本体で共通に進み続けるVSyncカウンタから現在の再生tickを決めます。トリガーを押したり離したりしても、この共通カウンタはリセットされません。", "再生位置は「floor(VSyncカウンタ ÷ Frame Step) mod マクロの総tick数」です。たとえばFrame Stepが1、総tick数が4なら、共通位相は0→1→2→3→0と進みます。4 tickの「A・無・無・無」と「無・B・無・無」を別々のボタンへ割り当てると、Aは位相0、Bは位相1で出力されます。どちらを先に押しても、この位置関係は変わりません。", "位相2で初めてボタンを押した場合、tick 0から開始せず、そのフレームからtick 2の出力を使います。マクロが無効な間は何も出力しませんが、共通位相は進み続けます。Frame Stepが2なら同じtickが2 VSyncフレームずつ続きます。総tick数が異なるマクロは同じ時計を参照しますが、それぞれ自身の総tick数で余りを取るため、同じtick番号に揃うわけではありません。", "ループ同期ではシーケンス全体を1周期として使います。ONにすると再生方法は「押している間反復」、ループ開始はstep 1へ変更され、同期中はループ開始を変更できません。途中のstepからだけを同期ループさせる動作はありません。通常の部分ループを使う場合はループ同期をOFFにします。", "「離したとき：すぐに中断」なら離したフレームで停止します。「現在の再生を完了」なら現在の同期周期の終端まで出力し、次のtick 0を出す直前に停止します。"] },
  ...
  en: {
    ...
    concepts: [
      ...
      { title: "Loop Sync", paragraphs: ["With normal repeat playback, the frame on which you press the trigger becomes tick 0. Loop Sync does not count from the trigger time. It derives the current playback tick from a shared VSync counter that keeps advancing in EASY ARCADE. Pressing or releasing a trigger does not reset this shared counter.", "The playback position is floor(VSync counter ÷ Frame Step) mod the macro's total tick count. For example, with Frame Step 1 and a four-tick macro, the shared phase advances 0→1→2→3→0. Assigning the four-tick patterns A– – – and – B– – to separate triggers makes A output at phase 0 and B at phase 1. Their relative timing remains the same regardless of which trigger was pressed first.", "If a trigger is first pressed during phase 2, the macro does not restart at tick 0; it immediately uses the output at tick 2. An inactive macro produces no output, but the shared phase keeps advancing. With Frame Step 2, each tick lasts for two VSync frames. Macros with different total tick counts share the same clock but apply different moduli, so their tick numbers are not generally identical.", "Loop Sync uses the entire sequence as one cycle. Turning it on changes playback to Repeat while held, sets the loop start to step 1, and prevents changing the loop start while synchronization is active. It does not support synchronizing only a partial loop that begins at a later step. Turn Loop Sync off to use a normal partial loop.", "Stop immediately ends output on the release frame. Finish playback continues through the end of the current synchronized cycle and stops immediately before the next tick 0 would be output."] },
