...
export function HelpContent() {
  ...
  return <main className="help-shell">
    ...
    <article className="help-content">
      ...
      <section className="help-section loop-sync-column" id="loop-sync"><span className="help-eyebrow">{page.loopSync.eyebrow}</span><h2>{page.loopSync.title}</h2><p>{page.loopSync.lead}</p><div className="sync-pattern"><table><thead><tr><th>{page.loopSync.phase}</th>{[1, 2, 3, 4].map((phase) => <th key={phase}>{phase}</th>)}</tr></thead><tbody>{page.loopSync.rows.map((row) => <tr key={row.name}><th>{row.name}</th>{row.values.map((value, index) => <td key={index}>{value}</td>)}</tr>)}</tbody></table></div><p>{page.loopSync.caption}</p><details><summary>{page.loopSync.detailsLabel}</summary><div>{page.loopSync.details.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></details></section>
