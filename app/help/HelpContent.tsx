...
const content = {
  ja: {
    ...
    concepts: [
      ...
      { title: "論理ボタンと出力", paragraphs: ["論理ボタンはCOIN、START、方向、A〜Lの18個です。ゲーム基板出力はCOIN、START、方向、A〜Fの12個です。G〜Lは直接対応する基板出力を持たず、マクロやステートセレクタの操作に使える仮想ボタンです。", "1つの論理ボタンから複数の基板出力を同時に出せます。たとえば論理ボタンGを、基板出力のA＋Bへ割り当てられます。複数の論理ボタンやマクロが同じ出力を生成した場合は、すべてOR合成されます。"] },
      { title: "直接マッピングとマクロ", paragraphs: ["直接マッピングは、論理ボタンを押している間、割り当てた基板出力をそのまま出します。マクロは、ボタンをきっかけにtick単位の出力シーケンスを再生します。マクロを起動したボタンの直接マッピングも同時に有効です。", "プロファイル全体で「1 tickを何フレームにするか」を設定できます。たとえば2なら、すべてのマクロで1 tickを2フレーム出力してから次へ進みます。", "1つのマクロを複数の論理ボタンへ割り当てることも、1つの論理ボタンから複数のマクロを同時に起動することもできます。左右・上下反転や再生方法は、キー割り当てごとに設定されます。"] },
      ...
      { title: "プロファイル", paragraphs: ["これらの設定をまとめたものがプロファイルです。プロファイルを.eamacroファイルへエクスポートし、USBメモリなどを介して読み込ませることで、編集した設定をEASY ARCADE実機へ伝えられます。", "Profileボタンの「サンプルから作成」では、用意されたプロファイルを新しい編集対象として追加できます。サンプルのタイミングは、使用するゲーム基板に合わせて調整してください。"] },
    ] as Section[],
    editorsTitle: "編集画面",
  ...
  en: {
    ...
    concepts: [
      ...
      { title: "Profiles", paragraphs: ["A profile contains all of these settings. Export it as an .eamacro file and transfer it through a USB drive or another storage device to load the edited settings into EASY ARCADE hardware.", "Use Create from Sample in the Profile menu to add a prepared profile as a new editable profile. Adjust sample timing for the target game board."] },
    ] as Section[],
