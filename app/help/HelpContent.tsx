...
const content = {
  ja: {
    ...
    concepts: [
      ...
      { title: "直接マッピングとマクロ", paragraphs: ["直接マッピングは、論理ボタンを押している間、割り当てた基板出力をそのまま出します。マクロは、ボタンをきっかけにtick単位の出力シーケンスを再生します。マクロの合成方式は、既存出力へ加えるOR、レバー入力を自動的に抑制する自動、任意の出力を抑制するカスタムから選べます。", "プロファイル全体で「1 tickを何フレームにするか」を設定できます。たとえば2なら、すべてのマクロで1 tickを2フレーム出力してから次へ進みます。ループ同期を有効にすると、起動時刻ではなくVSyncカウンタを基準にループ位置が決まり、同じ長さのマクロを別々に起動しても位相が揃います。", "1つのマクロを複数の論理ボタンへ割り当てることも、1つの論理ボタンから複数のマクロを同時に起動することもできます。左右・上下反転や再生方法は、キー割り当てごとに設定されます。シーケンス要約では、DOWN＋LEFT＋A＋Bのような同時出力を↙＋A＋Bとまとめて確認できます。"] },
  ...
  en: {
    ...
    concepts: [
      ...
      { title: "Direct mapping and macros", paragraphs: ["A direct mapping holds its assigned outputs while the logical button is held. A macro starts an output sequence measured in ticks. Its composition can add to existing outputs with OR, automatically suppress lever input, or suppress a custom set of outputs.", "The profile defines how many frames make up one tick. With a value of 2, every macro holds each tick for two frames before advancing. Loop Sync derives the loop position from the VSync counter, so separately triggered macros of the same length remain in phase.", "A macro can be assigned to several logical buttons, and one logical button can start several macros at once. Horizontal and vertical mirroring and playback behavior are configured per assignment. The sequence summary condenses simultaneous outputs such as DOWN+LEFT+A+B into ↙+A+B."] },
