...
EASY ARCADE本体で使用する1P・2Pボタン出力、連射オーバーライド、tick単位のマクロ、ステートセレクタを編集するブラウザアプリです。物理ボタンから論理ボタンへの割り当ては本体側で設定し、連射は本体設定の継承・無効化・プロファイル上書きを選べます。1 tickあたりのフレーム数はプロファイル内の全マクロで共通です。
