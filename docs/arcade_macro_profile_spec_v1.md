...
`vsyncCounter`は全Loop Syncバインディングが共有する単調増加カウンタとし、個別のシーケンスの起動、停止、再トリガではリセットしない。バインディングが停止中でもカウンタは進み続け、停止中のバインディングは出力だけを行わない。
...
Loop Syncは保持中反復が有効で、`Loop Start Step`が0のバインディングにだけ指定できる。Loop Syncを有効にする操作は保持中反復を有効にし、`Loop Start Step`を0へ変更する。Loop Syncが有効な間は`Loop Start Step`を変更できず、シーケンス全体を1周期として扱う。途中ステップからの部分ループとLoop Syncは併用しない。起動フレームではその時点の`playTick`に対応する出力を即時使用し、以後もVSyncカウンタから位置を決める。リリース時即時中断が有効ならリリースフレームで停止し、無効なら現在の同期周期がtick 0へ戻る直前まで再生して停止する。実行中インスタンスへの再トリガは従来どおり無視する。
...
| バージョン | 状態 | Sequence Binding | Sequence Definition | State Selector | 追加された動作 |
|---|---|---:|---:|---:|---|
| v1.0 | 旧ファームウェア向け正式形式 | 4-byte、Flags bit 0〜3 | 4-byteヘッダ | 10-byteヘッダ | 基本マクロ、反転、マクロセット |
| v1.1 | 現行の正式形式 | 4-byte、Flags bit 4にLoop Sync | 8-byteヘッダ | 13-byteヘッダ | 合成方式、抑制マスク、Loop Sync、セレクタ占有マスク |

Profile JSONの`schemaVersion`はこのMajor/Minorと独立する。`.eamacro`のバージョンは実機用バイナリのデコード規則、`schemaVersion`は編集用JSONの構造を表す。

### ランタイムのバージョン判定

EASY ARCADE本体は、次の順序と規則で読み込む。

1. Magic、Header Size、Total Size、CRC32を検証する
2. Majorが未対応なら、MinorやTLVを解釈せずファイル全体を拒否する
3. Major=1では、実装が明示的に対応するMinorだけを受理する。将来の未知Minorを既知セクションの長さから推測して読み込んではならない
4. Minor確定後、そのバージョンで定められた固定レコード長を使って全セクションを検証する。ランタイムは4-byteと8-byteの候補を試行して形式を推測しない
5. 全セクション、参照関係、予約ビット、実装上限の検証が完了してから、共通のランタイム構造へ展開する
6. 途中で失敗した場合は、現在適用中のプロファイルと実行状態を変更しない

未知TLV TypeはLengthに従ってスキップできるが、未知Minorを受理してよいことを意味しない。既知Typeのレコード配置がMinor間で変わるため、ファイルバージョンの明示判定を優先する。

### バージョン間マイグレーション

EASY ARCADE本体、PCエディタ、その他の互換実装は、読み込んだファイルを次の同一規則で現行の内部構造へ移行する。

| 入力 | Binding Loop Sync | Sequence Composition | Sequence Suppression Mask | Selector Occupancy Mask |
|---|---|---|---|---|
| 正式v1.0 | `false` | `自動` | シーケンスの全ステップから第6.1節の規則で算出 | `0` |
| 正式v1.1 | Playback Flags bit 4 | Composition Mode | Suppression Mask | Occupancy Mask |

移行後の実行結果は、読み込み主体が本体かPCエディタかによって変えてはならない。PCエディタの次回の通常書き出しは正式v1.1とする。ユーザーが明示的に旧形式書き出しを選んだ場合だけv1.0を生成し、v1.0で表現できない設定は暗黙に削除せず書き出しを拒否する。
...
ORではSuppression Maskを0とする。自動ではシーケンス全体から算出した値と一致しなければならない。カスタムでは任意の有効な実基板出力マスクを指定できる。v1.0の定義ヘッダはSequence ID、Step Count、Loop Start Step、Reservedの4 bytesとし、欠落する合成方式と抑制マスクは「バージョン間マイグレーション」の規則で補う。
...
標準の実機向け書き出しは最新のv1.1 `.eamacro`とする。ファイル操作メニューの下部に、旧ファームウェア用v1.0書き出しを控えめな互換機能として設ける。v1.0書き出し後に第10章のマイグレーションを適用した結果が現在の合成結果と一致しない場合、またはLoop Sync、占有マスクが使われている場合は、設定を暗黙に削除せず該当理由を示して書き出しを拒否する。具体的には、各マクロの実効抑制マスクが自動算出値と一致する場合だけv1.0へ書き出してよい。Profile JSONの書き出しも提供し、対応ブラウザではOSの保存ダイアログを開いて、ユーザーが任意の保存先とファイル名を選択できるようにする。非対応ブラウザでは通常のファイルダウンロードへフォールバックする。
...
合成方式はマクロ編集内で`OR`、`自動`、`カスタム`から選択する。「設定対象」のようなトリガー選択UIは設けない。新しいマクロの初期値は`自動`とする。`自動`では算出された抑制対象を表示し、`カスタム`へ切り替えた際はその自動値を初期値として引き継いでよい。`ループ同期`は保持中反復と同じ設定列に置き、現在のセットでそのマクロを起動する全トリガーへ同じ値を適用する。ループ同期中はループ開始ステップを1に固定し、入力欄を無効にする。
...
正式バージョン間の読み込みは、第10章「バージョン間マイグレーション」を正本とする。PCエディタの通常保存は、8-byte Sequence Definition、Loop Sync対応4-byte Sequence Binding、Occupancy Mask付きState Selectorsを持つ正式v1.1とし、明示的な旧形式エクスポートだけが正式v1.0を生成する。
...
- ランタイムがMajor/Minorを先に判定し、未知バージョンを既知レコード長から推測せず原子的に拒否する
- v1.0を本体、PCエディタ、互換実装のいずれへ読み込んでも、Loop Sync無効、合成方式自動、算出済み抑制マスク、占有マスク0の同じ内部状態へ移行する
- 正式v1.1の全追加フィールドが値を変えずに読み書きできる
- 連射パルスがシーケンスを再トリガしない
